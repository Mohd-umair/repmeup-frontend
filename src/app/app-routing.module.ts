...
const routes: Routes = [
  ...
  // Protected routes
  {
    ...
    children: [
      ...
      { path: 'contacts/segments', loadComponent: () => import('./features/contacts/segments/segments.component').then(m => m.SegmentsComponent), canActivate: [PermissionGuard], data: { permissions: ['contacts.read', 'inbox.read'] } },
      { path: 'contacts/duplicates', loadComponent: () => import('./features/contacts/duplicates/duplicates.component').then(m => m.DuplicatesComponent), canActivate: [PermissionGuard], data: { permissions: ['contacts.merge', 'inbox.read'] } },
      { path: 'contacts/fields', loadComponent: () => import('./features/contacts/custom-fields/custom-fields.component').then(m => m.CustomFieldsComponent), canActivate: [PermissionGuard], data: { permissions: ['customfields.manage', 'inbox.read'] } },
      { path: 'contacts/campaigns/new', redirectTo: 'campaigns', pathMatch: 'full' },
      { path: 'contacts/campaigns/:id', redirectTo: 'campaigns', pathMatch: 'full' },
      { path: 'contacts/campaigns', redirectTo: 'campaigns', pathMatch: 'full' },
      { path: 'contacts', loadComponent: () => import('./features/contacts/contacts-container/contacts-container.component').then(m => m.ContactsContainerComponent), canActivate: [PermissionGuard], data: { permissions: ['contacts.read', 'inbox.read'] } },
